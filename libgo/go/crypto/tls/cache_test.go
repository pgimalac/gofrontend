...
func TestWeakCertCache(t *testing.T) {
	if runtime.Compiler == "gccgo" {
		// This test relies on a cache entry being collected promptly
		// after runtime.GC. gccgo's conservative garbage collector does
		// not guarantee that weak pointers are cleared / cleanups run
		// promptly (or at all) for stack-referenced objects, so the
		// ref count never drops and the test times out. Skip it, as
		// gccgo does for other finalizer-dependent tests.
