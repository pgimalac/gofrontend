...
// switchToCrashStack0 switches to the crash stack and calls fn.
// gccgo does not have a dedicated crash stack (that is a gc-specific
// assembly mechanism), so just run fn directly. fn is expected to
// throw and not return.
...
//
// For gccgo the CPU feature variables (x86HasPOPCNT etc.) are provided by the
// compiler, not the runtime, so cpuinit only needs to extract GODEBUG and call
// cpu.Initialize.
func cpuinit() {
...
func kickoff() {
	...
	// for example from mcall(exitsyscallNoP) in exitsyscall, in
...
func oneNewExtraM() {
	...
	// from traceback and stack scans since it isn't a
...
func dropm() {
	...
	mp.curg.atomicstatus.Store(_Gdead)
	mp.curg.gcstack = 0
	mp.curg.gcnextsp = 0
...
func handoffp(pp *p) {
	...
	if (traceEnabled() || traceShuttingDown()) && traceReaderAvailable() {
		startm(pp, false, false)
		return
	}
...
func exitsyscallNoP(gp *g) {
	...
	traceExitingSyscall()
	trace = traceAcquire()
	casgstatus(gp, _Grunning, _Gexitingsyscall)
...
func panicgonil() {
	getg().m.throwing = throwTypeUser // do not dump full stacks
...
func newproc(fn uintptr, arg unsafe.Pointer) *g {
	...
	if fn == 0 {
		_g_.m.throwing = throwTypeUser // do not dump full stacks
		throw("go of nil func value")
	}
	...
	*(*unsafe.Pointer)(unsafe.Pointer(&entry)) = unsafe.Pointer(&newg.entryfn)
	...
	if isSystemGoroutine(newg, false) {
		...
		// Only user goroutines inherit synctest bubbles and pprof labels.
		newg.bubble = _g_.bubble
		if _g_.m.curg != nil {
			newg.labels = _g_.m.curg.labels
		}
		if goroutineProfile.active {
	...
	// fips140 bubble
	newg.fipsOnlyBypass = _g_.fipsOnlyBypass

	// dit bubble
	newg.ditWanted = _g_.ditWanted

	// Set up race context.
	if raceenabled {
		newg.racectx = racegostart(getcallerpc())
		newg.raceignore = 0
		if newg.labels != nil {
...
var expectedSystemGoroutines atomic.Uint32
...
func expectSystemGoroutine() {
	expectedSystemGoroutines.Add(1)
}
...
func waitForSystemGoroutines() {
	for expectedSystemGoroutines.Load() > 0 {
		Gosched()
		osyield()
	}
}
...
func gfput(pp *p, gp *g) {
	...
	if pp.gFree.size >= 64 {
		...
		sched.gFree.list.pushAll(q)
		sched.gFree.n += q.size
...
func gfget(pp *p) *g {
retry:
	if pp.gFree.empty() && !sched.gFree.list.empty() {
		lock(&sched.gFree.lock)
...
func (pp *p) destroy() {
	...
	pp.gcAssistTime = 0
	// gccgo does not track a per-P cleanup queue (pp.cleanupsQueued),
	// so there is no global cleanup queue to update here.
	sched.goroutinesCreated.Add(pp.goroutinesCreated)
