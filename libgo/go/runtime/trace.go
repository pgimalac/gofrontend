...
// It is a thin wrapper around gccgo's flat trace.enabled field, kept for
// compatibility with the gc 1.21 call sites elsewhere in the runtime.
//
//go:nosplit
...
// traceBlockReason is the type of the reason a goroutine blocks, kept for
// compatibility with the new tracer's API (see trace2runtime.go). In gccgo's
// flat byte-based tracer it is just the traceEvGo* event byte.
...
func traceReadCPU() {
	...
	for {
		...
		for len(data) > 0 {
			...
			// gccgo represents stack frames as locations rather than
			// raw PCs, so build locations from the sampled PCs.
			n := len(stk)
			if n > len(buf.stk) {
				n = len(buf.stk)
			}
			for i := 0; i < n; i++ {
				buf.stk[i] = location{pc: uintptr(stk[i])}
			}
			stackID := trace.stackTab.put(buf.stk[:n])
...
func (_ traceLocker) STWStart(reason stwReason) {
	...
	traceEvent(traceEvGCSTWStart, -1, uint64(reason))
}
...
// GoSwitch is used by the coroutine switch (coroswitch_m). The old
// (v1) execution tracer has no dedicated event for coroutine switches,
// so this is a no-op for gccgo.
...
func traceClockNow() traceTime {
	return traceTime(cputicks() / traceTickDiv)
}
